...
import { initials, type Speaker } from "@/lib/content/speakers";
...
export async function Speakers() {
  ...
  const allSpeakers = dynamicSpeakers;
  ...
  return (
    <Section id="speakers" tone="dark">
      ...
      {allSpeakers.length > 0 ? (
        ...
        <Reveal className="py-12 text-center border border-white/10 rounded-2xl bg-white/5">
          <Mic2 className="size-10 mx-auto text-white/40 mb-4" />
          <h3 className="text-xl font-semibold text-white">Speaker Lineup Coming Soon</h3>
          <p className="text-white/60 mt-2">We are currently reviewing applications and will announce our confirmed speakers shortly.</p>
        </Reveal>
      )}
